...
The library is written in #box[C/C++] programming language and is not tied to any particular game engine, meaning it can be plugged into different engines and custom game projects. Tests show that its results closely match professional ballistic calculators, and it runs fast enough to handle over a thousand projectiles simultaneously.


= Introduction

According to data published by the analytics firm Newzoo in 2025 @newzoo-report-2025, although the mobile segment dominates the global video game market with 83% of the total gaming audience, the PC platform still ranks second, accounting for 26% of all gamers. Within the PC segment, the shooter genre leads in revenue generation, comprising 23% of segment earnings, equivalent to approximately 9 billion USD @newzoo-report-2025. Shooting mechanics have also long transcended pure shooter games, becoming a standard gameplay element across diverse genres, from survival horrors to open-world action Role-Playing Games (RPGs). As a consequence, the fidelity of projectile behavior becomes a critical factor in shaping player perception: inconsistencies in projectile motion or impact response are immediately noticeable and directly affect perceived realism.

Modern game development employs two fundamental paradigms for simulating projectile motion, each reflecting a different balance between realism and technical simplicity. The first paradigm treats the projectile as an abstract point that reaches its target instantaneously along a straight path. Often referred to as a _hitscan_ model, this approach is prevalent in competitive multiplayer shooters where engagement distances are short and immediate hit registration is a design priority. The trajectory is reduced to a line-of-sight calculation between the weapon muzzle and the target, enabling rapid server–client synchronization and minimal network latency, at the cost of any physical behavior. Because the projectile is never modeled as a body, physical realism is entirely omitted, and this paradigm is not in the scope of this thesis. 

The second approach models the projectile as a body influenced by various physical forces during its flight, advancing its state over time. Although more computationally demanding, this method delivers higher physical fidelity and enables gameplay where weapon characteristics and surrounding conditions meaningfully influence the outcome. Unlike the hitscan paradigm, which sidesteps physics entirely, this approach places the task at the intersection of two domains. On one side, projectile motion is governed by well-established physical laws that have evolved from general classical dynamics into ballistics, a distinct branch of physics devoted to the behavior of objects launched into flight. On the other, embedding such a model into an interactive application is a software engineering problem, constrained by real-time execution, discrete-time integration, and the architectural conventions of modern game engines. The way this intersection is currently handled in practice, and the gaps it leaves, define the problem addressed in this thesis.
...
To address these issues, this thesis develops a ballistic simulation framework guided by three design principles, each addressing one of the aspects outlined above:

*Physical consistency.* Projectile motion is derived from established physical theory, with the projectile state advanced by numerical step-by-step integration under the active forces.

*Modular architecture.* Individual physical effects are encapsulated as independent components, allowing developers to assemble the simulation pipeline that matches the required level of realism, from simple parabolic motion suited for arcade games, through moderate realism for shooters, to high-fidelity modeling required by simulation-oriented titles.

*Engine independence.* Physical modeling is separated from rendering and other subsystems via a minimal and well-defined interface, enabling reuse across heterogeneous environments, including custom game projects, open-source game engines, and commercial platforms, without requiring significant modifications.

The framework is then has to be evaluated against these principles through a series of controlled simulations, in order to demonstrate the applicability of the proposed solution in practice.
...
// Kľúčové aspekty práce
Working on this thesis revealed two key aspects that were not anticipated at the outset. The first concerns the physical domain itself. The early stages proceeded without obstacle, because classical dynamics are extensively documented and broadly accessible. The transition occurred abruptly upon entering more specialized subdomains — in this project, at the examination of gyroscopic spin drift — where consumer-oriented sources cease to cover the subject. From that point onward, the available material is restricted to academic literature and domain-specific research papers that are not immediately transparent. The work shifts to prolonged study, in which the available formulations must be located across multiple sources, reconciled, and in some cases derived, before they can be reduced to code. The second difficulty was architectural. A disproportionate share of the development effort was spent not on physical equations, but on architectural decisions. Several substantial refactorings reorganized large portions of the codebase at once, and although each appeared at the time to be considerable overhead, these iterations were the mechanism through which the framework acquired its present form.

// Vplyv na spoločnosť
The broader societal relevance of this work follows from the scale of the genre to which it contributes. Shooter gameplay reaches a substantial share of the global player base, and projectile behavior within these titles shapes how a large audience perceives physical phenomena in a virtual setting. A recurring observation is that perceived realism depends strongly on effects that operate below the threshold of conscious recognition: players do not identify a projectile as behaving realistically because they recognize the air resistance or the gyroscopic drift, but because the absence of these effects makes everything else feel slightly inconsistent without a nameable cause. A framework that handles these effects by default, and leaves the decision of which to activate to the developer, contributes to an environment in which the default expectation for ballistic behavior in games shifts closer from approximation to physical realism.

// Vplyv na inžiniersku prax
The engineering contribution of this work addresses the structural gap in game development tooling identified in the problem statement. Beyond projectile physics, the architectural principles that emerged during development — the separation between data providers and force consumers, coordinate conventions as a configurable boundary, and the externalization of collision-specific information — reflect a broader position on how a specialized physics subsystem should relate to its host environment. These principles are not specific to ballistics, and their adoption in adjacent subsystems could contribute to a more universal approach to physics in interactive applications, where the current prevailing pattern is tight coupling.

// Hodnota výsledku pre cieľového používateľa
By lowering the barrier to integrating physically consistent ballistic behavior into interactive applications, the framework makes this capability available beyond studios able to support a custom physics pipeline. Small teams and solo developers can access the same physical foundation without months of ground-up implementation, broadening the range of projects in which ballistics is not reduced to a simplified parabola. Because fidelity is configured through component selection, developers can iterate on the model without fixing it at the architectural stage. Larger studios can adopt the framework and extend it with proprietary modules, or contribute directly into its current shape. Much as PhysX and Jolt Physics made rigid body dynamics broadly reusable, a dedicated ballistic framework could play a similar role for projectile physics in shooter gameplay.

// Prínos pre odborný rast autora
From the perspective of the author, the project consolidated a long-standing interest in core game engineering — the engineering of the technologies that make games possible. This domain traditionally encompasses graphics engines, physics engines, and comprehensive game engines. The work on this thesis provided an opportunity to engage with these areas in depth. Developing a physics library grounded in scientific sources required translating academic material into working code, which is a skill that rarely develops through coursework alone and is one of the more durable outcomes of the project for the author personally.

// Ako projekt obohatil pracovné prostredie autora
What distinguished this project from previous ones was the effort required to deliver a finished artifact. Writing a test suite, configuring the pipeline, producing documentation, preparing release packaging, and conducting evaluation were all part of the work, and each required attention that the core implementation alone would not have demanded. In retrospect, this shifted the outcome from a working code to a product that another person could realistically pick up and use. The usability study in particular altered the author's perspective on design decisions: technical work produced in isolation develops blind spots that only external use can expose, and the willingness to subject one's work to such exposure is a skill that has to be cultivated.

// Širší dopad práce
The more interesting implication of this work may ultimately concern not ballistics itself but the general pattern it demonstrates. A number of specialized physical domains — vehicle physics, structural deformation, fluid approximations — currently exist in a similar state: covered by academic literature, embedded as proprietary systems, but absent from the open-source landscape. The approach taken here: to study a physical model, isolate it behind minimal interfaces, decouple it from any specific runtime, and release it as a modifiable artifact, is not specific to projectile physics. The work therefore sits at the intersection of physics and software engineering, and the hope is that it contributes, however modestly, to a software ecosystem in which advanced physical simulation is treated as shared infrastructure.
